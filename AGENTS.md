...
- 先阅读 `docs/README.md` 判断文档权威顺序和当前阅读路径。
- `AGENTS.md` 只记录长期有效的开发约束；不要把阶段计划、临时结论或开发流水账塞进这里。
- 产品路线以 `docs/plan/roadmap.md` 和最新 active/planned phase 文档为准。
- 用户用法以 `docs/user-guide.md` 为准；未实现功能不要写成已可用。
- `logs/development/` 是开发过程记录，只能作为追溯材料，不应单独作为当前规范。
